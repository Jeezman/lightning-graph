...
export const SocketContext = React.createContext<{ socket: WebSocket }>({ socket: new WebSocket("ws://localhost:3000/ws") });

export const SocketProvider = ({ children }: { children: any }) => {
    // Persist socket instance between renders using useRef hook
    const socket = useRef(new WebSocket("ws://localhost:3000/ws"));
